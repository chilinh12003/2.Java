...
public class MyText
{
	private static String[] VietnameseSigns = new String[]{"aAeEoOuUiIdDyY", "ấầậẩẫắằặẳẵáàạảãâă", "ẤẦẬẨẪẮẰẶẲẴÁÀẠẢÃÂĂ",
			"ếềệểễéèẹẻẽê", "ẾỀỆỂỄÉÈẸẺẼÊ", "ốồộổỗớờợởỡóòọỏõôơ", "ỐỒỘỔỖỚỜỢỞỠÓÒỌỎÕÔƠ", "ứừựửữúùụủũư", "ỨỪỰỬỮÚÙỤỦŨƯ",
	 ...
	 * 
	 * @param Type
	 *            : Type = 1: Xoa tat ca, chi giu lai ky tu so
	 *            <p>
	 *            Type = 2: xóa tất cả chỉ giữ lại ký tự chữ và số.
	 *            </p>
	 *            <p>
	 *            Type = 3: xóa tất cả chỉ giử lại ký tự chữ
	 * @param Source
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * Xoa bo ky tu dac biet cua mot chuoi nhưng ko xóa bảo ký tự đặc biệt
	 * truyền vào
	 * 
	 * @param Type
	 *            : Type = 1: Xoa tat ca, chi giu lai ky tu so
	...
	public static String RemoveSpecialLetter(int Type, String Source, String SpecialLetter) throws Exception
	{
		try
		{
			...
			else if (Type == 2) // Giữ lại ký tự chữ và số
			{
				...
				{
					char ch = Source.charAt(i);
					if (SpecialLetter.indexOf(ch) >= 0 || (ch >= '0' && ch <= '9') || (ch >= 'A' && ch <= 'Z')
							|| (ch >= 'a' && ch <= 'z'))
					{
						Result += ch;
					}
	...
	/**
	 * Xóa ký tự đặc biệt ở đầu và cuối
	 * 
	 * @param sInput
	 * @return
	 */
	...
	public static Integer getShortSMSCount(String MTContent)
	{
		if (MTContent == null)
			return 0;
		Integer MTLength = MTContent.length();
		if (MTLength <= 160)
			return 1;
