...
public class ProRegister
{
	...
	private void CreateRegAgain() throws Exception
	{
		try
		{
			...
			if (mSubObj.StatusID == dat.sub.Subscriber.Status.UndoSub.GetValue())
			{
	...
	private int GetPartnerID() throws Exception
	{
		...
		// đối tác về
		if (Common.GetChannelType(Channel) == ChannelType.SMS)
		{
			try
			{
	...
	private void Update_WapRegLog()
	{
		try
		{
					...
					|| Common.GetApplication(AppName).mApp == VNPApplication.TelcoApplication.MOBILEADS)
			{
				...
				mTable_WapRegLog.SetValueAt(MyConfig.Get_DateFormat_InsertDB().format(mCal_Current.getTime()), 0, "RegisterDate");
