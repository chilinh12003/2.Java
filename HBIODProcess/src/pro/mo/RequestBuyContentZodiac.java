...
public class RequestBuyContentZodiac extends ContentAbstract
{
	...
	private void GetZodiac() throws Exception
	{
		String Info = mMsgObject.getUsertext().toUpperCase();
		
		String Zodiac_Keyword = Info.replace(mMsgObject.getKeyword().toUpperCase(), "");
		Zodiac_Keyword = MyText.RemoveSpecialLetter(3, Zodiac_Keyword, "");
		
		mZodiacObj = CurrentData.Get_ZodiacObj(Zodiac_Keyword);
	...
	protected Collection<MsgObject> getMessages(MsgObject msgObject, Keyword keyword) throws Exception
	{
		try
		{
			...
			{
				mLog.log.warn("Dịch vụ không tồn tại!");
				mMTType = MTType.RequestFail;
				return AddToList();
			}

			if (mZodiacObj.IsNull())
			{
				mLog.log.warn("Cung hoàng đạo không tồn tại!");
				mMTType = MTType.RequestFail;
				return AddToList();
			}
			...
			mNewsObj = CurrentData.GetNews(PID, mMsgObject.getUserid(), mServiceObj, mZodiacObj);
