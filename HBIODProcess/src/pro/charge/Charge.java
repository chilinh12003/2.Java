...
public class Charge
{
	...
	public enum ErrorCode
	{
		...
		ChargeSuccess(0), BlanceTooLow(1), ChargeNotComplete(2), UserNotExist(3), OtherError(4), InvalidSubscriptionState(
				5), VNPAPIError(-1);
	...
	public static ErrorCode BuyContent(Integer PartnerID, ServiceObject mServiceObj, NewsObject mNewsObj,
			String MSISDN, String Keyword, MyConfig.ChannelType mChannel) throws Exception
	{
		try
		{
			...
			{
				mResult = ChargeVNP(PartnerID, mServiceObj, mNewsObj, MSISDN, 1000, Reason.CONTENT, 1000, 0, Keyword,
						mChannel);
	...
	private static boolean InsertChargeLog(Integer PartnerID, ServiceObject mServiceObj, String MSISDN, Integer PRICE,
			Reason REASON, Integer ORIGINALPRICE, Integer PROMOTION, MyConfig.ChannelType CHANNEL, ErrorCode mResult)
	{
		try
		{
	...
	private static ErrorCode ChargeVNP(Integer PartnerID, ServiceObject mServiceObj, NewsObject mNewsObj,
			String MSISDN, Integer PRICE, Reason REASON, Integer ORIGINALPRICE, Integer PROMOTION, String NOTE,
			MyConfig.ChannelType CHANNEL) throws Exception
	{
		String XMLResponse = "";
		String XMLReqeust = "";
		...
		String ContentName = mServiceObj.PackageName;
		String ContentID = Integer.toString(mNewsObj.NewsID);
		...
		try
		{
			String Template = "<CCGWRequest servicename=\"%s\" username=\"%s\" password=\"%s\">"
					...
					+ "<CHANNEL>%s</CHANNEL>"
					+ "<Content>"
					+ "<item contenttype=\"CONTENT\" subcontenttype=\"VI\" contentid=\"%s\" contentname=\"%s\" cpname=\"%s\" note=\"\" playtype=\"DOWNLOAD\" contentprice=\"%s\"/>"
					+ "</Content>" + "</CCGWRequest>";

			XMLReqeust = String.format(Template, new Object[]{CPName, UserName, Password, RequestID, MSISDN, PRICE,
					REASON.toString(), ORIGINALPRICE, PROMOTION, NOTE, CHANNEL.toString(), ContentID, ContentName,
					CPName, PRICE});
			...
			if (!InsertChargeLog(PartnerID, mServiceObj, MSISDN, PRICE, REASON, ORIGINALPRICE, PROMOTION, CHANNEL,
					mResult))
			{
