...
public class ProRegister
{
	...
	ChannelType mChannel = ChannelType.NOTHING;
	...
	/**
	 * Tên đối tác do vinaphone truyền vào trường note
	 * VD: HBCOM|KMT6
	 */
	String PartnerKey = "";
	...
	private void SetPromotionToSub() throws Exception
	{
		...
		else if (mPromoObj.mAdvertiseType == AdvertiseType.Trial)
		{
			if (mPromoObj.mTrialType == TrialType.Day)
			{
				...
				if ((int) mPromoObj.TrialNumberFree == 1)
				{
					FreeTime = "ngay dau tien";
				}
				else
				{
		...
		else if (mPromoObj.mAdvertiseType == AdvertiseType.Promotion)
		{
			if (mPromoObj.mPromotionType == PromotionType.Day)
			{
				...
				if ((int) mPromoObj.PromotionNumberFree == 1)
				{
					FreeTime = "ngay dau tien";
				}
				else
	...
	private void CreateRegAgain() throws Exception
	{
		try
		{
			mSubObj.ChannelTypeID = mChannel.GetValue();
	...
	private int GetPartnerID() throws Exception
	{
		try
		{
			// Nếu là đăng ký qua 1 kênh sms quảng cáo nào đó, thì lấy thông tin
			// đối tác về
			if (this.mChannel == ChannelType.SMS)
			{
				try
				{
					return mKeyword.GetPartnerID(Keyword);
				}
				catch (Exception ex)
				{
					mLog.log.error(ex);
			...
			{
				/*
				 * WapRegLog mWapRegLog = new
				 * WapRegLog(LocalConfig.mDBConfig_MSSQL); mTable_WapRegLog =
				 * mWapRegLog.Select(2, mSubObj.MSISDN);
				 * 
				 * 
				 * if (mTable_WapRegLog != null &&
				 * mTable_WapRegLog.GetRowCount() > 0) {
				 * 
				 * return Integer.parseInt(mTable_WapRegLog.GetValueAt(0,
				 * "PartnerID").toString()); // return //
				 * GetPartnerID_Pass(Integer
				 * .parseInt(mTable_WapRegLog.GetValueAt(0, //
				 * "PartnerID").toString())); }
				 */

				Partner mPartner = new Partner(LocalConfig.mDBConfig_MSSQL);
				MyTableModel mTable_mPartner = mPartner.Select(5, PartnerKey);

				if (mTable_mPartner != null && mTable_mPartner.GetRowCount() > 0)
				{
					return Integer.parseInt(mTable_mPartner.GetValueAt(0, "PartnerID").toString());
				}
				// nêu la mobile ads thi tra ve san luong cua doi tac dang quang
				// cao
				return 0;
			}
		}
	...
	private MTType AddToList()
	{
		try
		{
			...
			if (mSubObj.AppID == VNPApplication.TelcoApplication.VASVOUCHER.GetValue())
			{
			...
			if (mMTType == MTType.RegNewSuccess || mMTType == MTType.RegAgainSuccessFree
					|| mMTType == MTType.RegAgainSuccessNotFree || mMTType == MTType.RegCCOSSuccessFree
					|| mMTType == MTType.RegCCOSSuccessNotFree || mMTType == MTType.RegVASDealerSuccessFree
					|| mMTType == MTType.RegVASDealerSuccessNotFree || mMTType == MTType.RegVASVoucherSuccessFree
					|| mMTType == MTType.RegVASVoucherSuccessNotFree || mMTType == MTType.RegMOBILEADSSuccessFree
					|| mMTType == MTType.RegMOBILEADSSuccessNotFree)
			{
				...
				{
					// nếu là mobile ads thì ko push mt tin tuc
					if (mSubObj.AppID != VNPApplication.TelcoApplication.MOBILE_ADS.GetValue()
							&& mSubObj.AppID != VNPApplication.TelcoApplication.MOBILEADS.GetValue())
					{
						Common.SendMT(MSISDN, Keyword, MTContent_Current_Push, RequestID);
					}
		...
		try
		{
			...
			if (mPromoObj.mAdvertiseType != AdvertiseType.Normal)
			{
				...
				if (mSubObj.IsNull())
				{
					...
					ErrorCode mResult = Charge.ChargeRegFree(mSubObj.PartnerID, MSISDN, Keyword, mChannel,
					...
					if (Insert_Sub())
					{
						...
						else if (mSubObj.AppID == VNPApplication.TelcoApplication.VASDEALER.GetValue())
						{
							mMTType = MTType.RegVASDealerSuccessFree;
						}
						else if (mSubObj.AppID == VNPApplication.TelcoApplication.VASVOUCHER.GetValue())
						{
							mMTType = MTType.RegVASVoucherSuccessFree;
						}
						else if (mSubObj.AppID == VNPApplication.TelcoApplication.MOBILE_ADS.GetValue()
								|| mSubObj.AppID == VNPApplication.TelcoApplication.MOBILEADS.GetValue())
						{
			...
			if (mSubObj.IsDereg)
			{
				...
				ErrorCode mResult = Charge.ChargeReg(mSubObj.PartnerID, MSISDN, Keyword, mChannel,
						Common.GetApplication(AppName), UserName, IP);
