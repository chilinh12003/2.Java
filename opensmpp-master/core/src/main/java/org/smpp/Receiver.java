...
/**
 * <code>Receiver</code> is class used for receiving PDUs from SMSC. It can be
 * used two ways: it has methods for synchronous (blocking) receiving of PDUs
 * and as it is derived from <code>ReceiverBase</code> whic on turn is derived
 * from <code>ProcessingThread</code> class, it can also receive PDUs on
...
{
	/**
	 * Name of the thread created when starting the
	...
	/**
	 * The correspondent transmitter for transmitting PDUs. It's used for
	 * sending of generic negative acknowledges, if necessary. It is passed to
	 * the receiver as a parameter during construction.
	 * 
	 * @see #receiveAsync()
	 */
	...
	/**
	 * The network connection which is used for receiving data. It is passed to
	 * the receiver as a parameter during construction.
	 */
	private Connection connection = null;

	/**
	 * The queue which holds the received PDUs. As the PDUs are received in
	 * asynchronnous manner, they are stored to a queue from which they can be
	 * get using method <code>receive</code>. PDUs are stored to the queue if
	 * and only if the <code>Receiver</code> is started as a separate thread
	...
	/**
	 * This timeout specifies for how long will go the receiving into wait if
	 * the PDU (expected or any) isn't in the <code>pduQueue</code> yet. After
	 * that the queue is probed again (etc.) until receiving timeout expires or
	 * the PDU is received.
	 * 
	...
	/**
	 * This object holds data received from connection which aren't complete PDU
	 * yet. If this situation occurs, it's likely that the data will be received
	 * the next time when another attempt to receive data from the connection
	 * occurs. Its used in <code>ReceiverBase</code>'s
	...
	/**
	 * If true then GenericNack messages will be sent automatically if message
	 * can't be parsed If false then the pdu will be submitted to listener for
	 * processing and submission to peer
	 */
	...
	/**
	 * This constructor sets the connection to receive the messages from and a
	 * transmitter for sending generic negative acknowledges if necessary.
	 * 
	 * @param transmitter
	...
	/**
	 * Sets the event listener for asynchronous <code>Receiver</code>. In case
	 * there are unprocessed PDUs in the queue, they are removed from the queue
	 * and passed to the newly set listener.
	 */
	...
	/**
	 * This method receives a PDU or returns PDU received on background, if
	 * there is any. It tries to receive a PDU for the specified timeout. If the
	 * receiver is asynchronous, then no attempt to receive a PDU and
	 * <code>null</code> is returned. The function calls are nested as follows:<br>
	 * <ul>
	 ...
	 * and the ReceiverBase.run() function which actually receives the PDUs and
	 * stores them to a queue looks as follows:<br>
	 * <code>
	 *       ReceiverBase.run()<br>
	 ...
	 */
	public synchronized PDU receive(long timeout) throws UnknownCommandIdException, TimeoutException,
			NotSynchronousException, PDUException, IOException
	{
		PDU pdu = null;
		if (!asynchronous)
		{
	 ...
	 * 
	 * @param expectedPDU
	 *            the template for expected PDU; the PDU returned must have the
	 *            same sequence number
	 * @return the received PDU or null if none
	 * @see ReceiverBase#tryReceivePDUWithTimeout(Connection,PDU,long)
	 */
	public synchronized PDU receive(PDU expectedPDU) throws UnknownCommandIdException, TimeoutException,
			NotSynchronousException, PDUException, IOException
	{
		PDU pdu = null;
		if (!asynchronous)
	...
	/**
	 * This method tries to receive one PDU from the connection. It is called in
	 * cycle from <code>tryReceivePDUWithTimeout</code> until timeout expires.
	 * <code>tryReceivePDUWithTimeout</code> is called either from
	 * <code>receiveAsync</code> as asynchronous receive on background or from
	 * <code>receive</code> as synchronous receive. It either gets pdu from the
	 * queue or tries to receive it from connection using
	 * <code>receivePDUFromConnection</code> depending on the value of the
	 * <code>receiver</code> flag. The method checks if the actualy received PUD
	 ...
	 */
	protected PDU tryReceivePDU(Connection connection, PDU expectedPDU) throws UnknownCommandIdException,
	 ...
	 * <code>pduQueue</code>. It's called from the <code>ReceiverBase</code>'s p
	 * <code>process</code> method which is called in loop from
	 * <code>ProcessingThread</code>'s <code>run</code> method.
	 * <p>
	 * If an exception occurs during receiving, depending on type of the
	 * exception this method either just reports the exception to debug & event
	 * objects or stops processing to indicate that it isn't able to process the
	 * exception. The function <code>setTermException</code> is then called with
	 ...
	 * 
	 * @param pdu
	 *            the PDU to pass to the processor as an
	...
	/**
	 * Sends <code>GenericNack</code> PDU via transmitter if there is one. The
	 * <code>GenericNack</code> is sent in case that the message is corrupted or
	 * has unknown command id. If the sending of <code>GenericNack</code> fails,
	 * this method calls <code>stopProcessing</code> and thus stops the
	 * receiving thread.
	 * 
	 * @param commandStatus
	 *            the error code
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.1 2003/07/23 00:28:39 sverkera
 * Imported
 * 
 * 
 * Old changelog: 13-07-01 ticp@logica.com start(), stop(),
 * setQueueWaitTimeout() & getQueueWaitTimeout() made not synchronized;
 * receive(long) & receive(PDU) made synchronized so the receiver no longer
 * locks up 13-07-01 ticp@logica.com bug fixed in tryReceivePDU which caused
 * that the PDUs were never removed from the queue - now dequeue(expected) is
 * now used instead of find(expected) 13-07-01 ticp@logica.com loads of debug
 * lines corrected; some added 08-08-01 ticp@logica.com added support for
 * Session's asynchronous processing capability 26-09-01 ticp@logica.com debug
 * code categorized to groups 01-10-01 ticp@logica.com added function
 * getThreadName for ProcessingThread thread name initialisation. 02-10-01
 * ticp@logica.com instead of importing full packages only the used classes are
 * iported
 */
