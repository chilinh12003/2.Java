...
public class Session extends SmppObject
{
	 ...
	 */
	final public BindResponse bind(BindRequest bindReq) throws ValueNotSetException, TimeoutException, PDUException,
	 ...
	 * @see OutbindReceiver
	 */
	final public void outbind(Outbind request) throws ValueNotSetException, TimeoutException, PDUException,
			IOException, WrongSessionStateException
	{
		checkState(request);
		send(request);
	 ...
	 * @see UnbindResp
	 */
	final public UnbindResp unbind() throws ValueNotSetException, TimeoutException, PDUException, IOException,
	 ...
	 * @see GenericNack
	 */
	final public void genericNack(GenericNack response) throws ValueNotSetException, TimeoutException, IOException,
			WrongSessionStateException
	{
		checkState(response);
		try
	 ...
	 * @see GenericNack
	 */
	final public void genericNack(int commandStatus, int sequenceNumber) throws ValueNotSetException, TimeoutException,
			IOException, WrongSessionStateException
	{
	 ...
	 */
	final public SubmitMultiSMResp submitMulti(SubmitMultiSM request) throws ValueNotSetException, TimeoutException,
			PDUException, IOException, WrongSessionStateException
	{
		checkState(request);
	 ...
	 * @see DeliverSMResp
	 */
	final public DeliverSMResp deliver(DeliverSM request) throws ValueNotSetException, TimeoutException, PDUException,
			IOException, WrongSessionStateException
	{
		checkState(request);
	 ...
	 * @see DataSMResp
	 */
	final public DataSMResp data(DataSM request) throws ValueNotSetException, TimeoutException, PDUException,
			IOException, WrongSessionStateException
	{
		checkState(request);
	 ...
	 * @see QuerySMResp
	 */
	final public QuerySMResp query(QuerySM request) throws ValueNotSetException, TimeoutException, PDUException,
			IOException, WrongSessionStateException
	{
		checkState(request);
	 ...
	 * @see CancelSMResp
	 */
	final public CancelSMResp cancel(CancelSM request) throws ValueNotSetException, TimeoutException, PDUException,
			IOException, WrongSessionStateException
	{
		checkState(request);
	 ...
	 * @see ReplaceSMResp
	 */
	final public ReplaceSMResp replace(ReplaceSM request) throws ValueNotSetException, TimeoutException, PDUException,
			IOException, WrongSessionStateException
	{
		checkState(request);
	 ...
	 * @see EnquireLinkResp
	 */
	final public EnquireLinkResp enquireLink(EnquireLink request) throws ValueNotSetException, TimeoutException,
			PDUException, IOException, WrongSessionStateException
	{
		checkState(request);
	...
	/**
	 * Simplified veriosn of <a
	 * href="#enquireLink(EnquireLink)">enquireLink</a>. As the
	 * <code>EnquireLink</code> PDU doesn't contain any parameters axcept of
	 * header, there is might be no need to provide the PDU as a parameter to
	 ...
	 * @see EnquireLinkResp
	 */
	final public EnquireLinkResp enquireLink() throws ValueNotSetException, TimeoutException, PDUException,
			IOException, WrongSessionStateException
	{
	 ...
	 */
	final public void alertNotification(AlertNotification request) throws ValueNotSetException, TimeoutException,
			PDUException, IOException, WrongSessionStateException
	{
		checkState(request);
		send(request);
	 ...
	 * @see ReceiverBase
	 */
	final public PDU receive() throws UnknownCommandIdException, TimeoutException, NotSynchronousException,
			PDUException, IOException
	{
		if (!asynchronous)
		{
	 ...
	 * @see ReceiverBase
	 */
	final public PDU receive(long timeout) throws UnknownCommandIdException, TimeoutException, NotSynchronousException,
			PDUException, IOException
	{
		PDU pdu = null;
	 ...
	 * @see Receiver
	 */
	final private Response send(Request request, boolean asynchronous) throws ValueNotSetException, TimeoutException,
			PDUException, IOException
	{
	 ...
	 */
	final private Response send(Request request) throws ValueNotSetException, TimeoutException, PDUException,
			IOException
	{
	 ...
	 * the class mathces, but the command id says, that the PDU is generic nack.<br>
	 ...
	 * @see #send(Request,boolean)
	 */
	private Response checkResponse(PDU pdu, Response expResponse) throws ValueNotSetException, TimeoutException,
			IOException
	{
		Response response = null;
	 ...
	 * The checking can be turned off, see <a
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.3 2006/02/22 16:45:01 paoloc
 * UnbindServerPDUEventListener: if (pdu.equals(unbindReq)): as reported by
 * users, this is incompatible with the new improved PDU.equals(); fixed.
 * 
 * Revision 1.2 2004/09/10 23:03:44 sverkera Added isOpened method
 * 
 * Revision 1.1 2003/07/23 00:28:39 sverkera Imported
 * 
 * 
 * Old changelog: 08-08-01 ticp@logica.com added asynchronous processing
 * capability 02-10-01 ticp@logica.com tracing now belongs to DSESS group
 * 20-11-01 ticp@logica.com the receiver thread is now started even for the
 * transmitter session - before transmitter sessions couldn't receive anything
 * 20-11-01 ticp@logica.com receiver thread is started after the bind request is
 * sent to the MC, before it was started before the bind req was sent to MC,
 * therefore the response could be processed by the listener 22-11-01
 * ticp@logica.com implemented session states with checking if certain operation
 * is allowed in the current session state
 */
