...
/**
 * Abstract base class for classes which can receive PDUs from connection. The
 * receiving of PDUs can be be performed on background within a separate thread
 * using the <code>ProcessingThread</code> class.
 * 
...
public abstract class ReceiverBase extends ProcessingThread
{
	...
	/**
	 * Timeout for receiving the rest of PDU from the connection. If the rest of
	 * PDU isn't receive in time, <code>TimeoutException</code> is thrown.
	 * 
	...
	/**
	 * Method repeatedly called from <code>process</code> method. It's expected
	 * that derived classes implement atomic receive of one PDU in this method
	 * using ReceiverBase's <code>tryReceivePDUWithTimeout</code> and
	...
	/**
	 * This method should try to receive one PDU from the connection. It is
	 * called in cycle from <code>tryReceivePDUWithTimeout</code> until timeout
	 * expires. The method should check if the actualy received PDU is equal to
	 * the <code>expectedPDU</code>.
	 * 
	 * @param connection
	 *            the connection from which the PDU should be received
	 * @param expectedPDU
	 *            the command id and sequence id of the received PDU should be
	 *            equal to those of expectedPDU
	 ...
	 */
	protected abstract PDU tryReceivePDU(Connection connection, PDU expectedPDU) throws UnknownCommandIdException,
			TimeoutException, PDUException, IOException;

	/**
	 * This is an implementation of <code>ProcessingThread</code>'s
	 * <code>process</code> method, which is method called in loop from the
	 ...
	 * calling method <code>tryReceivePDU</code>. The method
	 * <code>tryReceivePDU</code> must be implemented in the derived class.
	 * <p>
	 * The timeout can be either value > 0, then it means for how many
	 * milliseconds will be repeatedly tried to receive a PDU. If the timeout is
	 * = 0 then there is only one attempt to receive a PDU. If the timeout is
	 * equal to Data.RECEIVE_BLOCKING, then the this method tries receive a PDU
	...
	/**
	 * Elementary method receiving data from connection, trying to create PDU
	 * from them and buffering data in case the PDU isn't still complete. It has
	 * timeout checking for incomplete messages: if the message isn't received
	 * completly for certain time and no new data are received for this time,
	 * then exception is thrown as this could indicate communication problem.
	 * 
	 * @param connection
			...
			throws UnknownCommandIdException, TimeoutException, PDUException, IOException
	{
		...
		try
		{
			...
			{ // only if we didn't manage to get pdu from unproc
				...
				unprocBuffer = unprocessed.getUnprocessed();
				// if received something now or have something from the last
				// receive
				if (buffer.length() != 0)
				{
		...
		{
			// paolo@bulksms.com: if we got an UnknownCommandIdException here,
			// the
			// chances are excellent that some trailing garbage is hanging
			// around in
			// the unprocessed buffer. Given that it's unlikely that it
			// contained a
			// valid PDU, we don't rethrow the error - nothing to respond to
			// with a
			// gnack. Was originally just checking this if the unprocessed
			// buffer had
	...
	/**
	 * Tries to create a PDU from the buffer provided. Returns the PDU if
	 * successfull or null if not or an exception if the PDU is incorrect.
	 */
	private final PDU tryGetUnprocessedPDU(Unprocessed unprocessed) throws UnknownCommandIdException, PDUException
	{
		...
		catch (HeaderIncompleteException e)
		{
			...
			unprocessed.setHasUnprocessed(false); // as it's incomplete - wait
													// for new data
			unprocessed.setExpected(Data.PDU_HEADER_SIZE);
		}
		catch (MessageIncompleteException e)
		{
			// paolo@bulksms.com - this number (5) is somewhat arbitrary. Too
			// low
		...
		catch (UnknownCommandIdException e)
		{
			...
			// contains at least a header - see code in PDU) with an error,
			// which
			// is fine.
		...
		{
			// paolo@bulksms.com: safer to catch all other PDU exceptions and
			// force
			// force a check() here - some exception in parsing should not be
			// allowed
			// to leave ghost data in the Unprocessed buffer (even though this
			// is now
		...
		/*
		 * paolo@bulksms.com: concerned that this is too broad, and will stop
		 * useful exceptions from being passed back up the call stack, so
		 * disabling for now: } catch (Exception e) { debug.write(DRXTX,
		 * "Exception catched: " + e.toString()); StringWriter stringWriter =
		 * new StringWriter(); PrintWriter printWriter = new
	...
	/**
	 * Sets the timeout for receiving the complete message. If no data are
	 * received for time longer then this timeout and there is still not
	 * completly received PDU in the internal buffer,
	 ...
	 * <code>timeout</code> can indicate either timeout in milliseconds (if >
	 * 0), or that there has to be only one attempt to receive a message (if =
	 * 0) or that the the receiving shuld continue until a PDU is received (if =
	 * Data.RECEIVE_BLOCKING).
	 * 
	 * @param startTime
...
/*
 * $Log: not supported by cvs2svn $ Revision 1.3 2004/09/04 09:34:50 paoloc no
 * message
 * 
 * Revision 1.2 2004/09/04 09:00:27 paoloc Various changes which deal better
 * with invalid ((non-spec or garbage) data being received. Previously, such
 * data could get a Receiver object stuck in a state in which it stops passing
 * incoming data to the application.
 * 
 * Revision 1.1 2003/07/23 00:28:39 sverkera Imported
 * 
 * 
 * Old changelog: 13-07-01 ticp@logica.com start(), stop(), setReceiveTimeout(),
 * getReceiveTimeout(), setTermException(), getTermException() made not
 * synchronized; receive(long) & receive(PDU) made synchronized so the receiver
 * no longer locks up 13-07-01 ticp@logica.com the call to receiveAsync() in
 * run() now enclosed in try-finally and the status RCV_FINISHED is set in
 * finally block so the finished status is now reported correctly even in case
 * of exception 13-07-01 ticp@logica.com some debug lines corrected; some added
 * 08-08-01 ticp@logica.com added support for Session's asynchronous processing
 * capability 23-08-01 ticp@logica.com added async capability exhibited that if
 * more than one pdu is read from connection at once (in one
 * connection.receive(), the additional ones aren't processed. this behaviour
 * was corrected. 23-08-01 ticp@logica.com added yield() to run() to give chance
 * to other threads 26-09-01 ticp@logica.com debug code categorized to groups
 * 01-10-01 ticp@logica.com now derived from new ProcessingThread which
 * implements the thread related issues; this change allows to focus only on the
 * receiving related in the ReceiverBase and on the thread management in the
 * ProcessingThread; function covered by ProcessingThread removed
 */
