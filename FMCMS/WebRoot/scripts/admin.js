...
function ShowSubMenu(ctr_This)
{
    ...
    var top = $(ctr_This).offset().top + $(ctr_This).outerHeight();
    ...
    var min_sub_width = $(ctr_This).outerWidth();

    SetWidthChildMenu(ctr_SumMenu_Item, min_sub_width);

}
function SetWidthChildMenu(ctr_Parent, min_sub_width)
{
    if (!ctr_Parent)
        return;
...
function HideSubMenu(SubMenu_ID)
{
    ...
    DelayHideSupMenu = setTimeout("HideDelaySubMenu('" + SubMenu_ID + "')", 100);
}
