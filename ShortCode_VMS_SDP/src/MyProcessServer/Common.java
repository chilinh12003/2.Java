...
public class Common
{
	...
	public static int sendMT1(MsgObject msgObject)
	{
		...
		try
		{
			...
			{
				Result = mSendQueue.Insert_VMS(msgObject.getUserid(), msgObject.getServiceid(), msgObject.getMobileoperator(), msgObject.getKeyword(),
						msgObject.getUsertext(), null, null, Integer.toString(msgObject.getMsgtype()), msgObject.getRequestid().toString(), "1",
						Integer.toString(msgObject.getContenttype()), Integer.toString(msgObject.getCpid()),msgObject.getVMS_SVID());
			}
			else
			{
				Result = mSendQueue.Insert(msgObject.getUserid(), msgObject.getServiceid(),
