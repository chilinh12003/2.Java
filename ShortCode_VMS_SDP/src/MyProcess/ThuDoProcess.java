...
public class ThuDoProcess extends ContentAbstract
{
	...
	protected Collection<?> getMessages(MsgObject msgObject, Keyword keyword) throws Exception
	{
		...
		try
		{
			...
			try
			{
				...
				{
					Result = ThuDoForwardMO.CallWebservice(msgObject.getUserid(), msgObject.getServiceid(), msgObject.getKeyword(), msgObject.getUsertext(),
		...
		finally
		{
			try
			{
				mForward.Insert_VMS(msgObject.getUserid(), msgObject.getServiceid(), msgObject.getMobileoperator(), msgObject.getKeyword(), msgObject.getUsertext(),
					InsertDate, ReceiveDate, "0", msgObject.getRequestid().toString(), "", Integer.toString(msgObject.getCpid()), "0", Integer.parseInt(Result),RetryCount,msgObject.getVMS_SVID());
			}
			catch(Exception ex)
			{
