...
public class VideoProcess extends ContentAbstract
{
	...
	public VideoProcess()
	{
		try
		{
			mVideo = new Video(LocalConfig.mDBConfig_MSSQL_HBStore);
		}
		catch (Exception ex)
		{
	...
	protected Collection<MsgObject> getMessages(MsgObject msgObject, Keyword keyword) throws Exception
	{
		...
		try
		{
			...
			MyTableModel mTable = mVideo.Select(1, VideoID);
			...
			String Result = mGetMedia.getGetMediaSoap().getLinkMedia("6x83", "6x83sdadaffasasxa", msgObject.getUserid(), 3, VideoID, 3, Price, RequestTime);
