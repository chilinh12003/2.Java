...
public class ImageProcess extends ContentAbstract
{
	...
	public ImageProcess()
	{
		try
		{
			mImage = new Image(LocalConfig.mDBConfig_MSSQL_HBStore);
		}
		catch (Exception ex)
		{
	...
	protected Collection<MsgObject> getMessages(MsgObject msgObject, Keyword keyword) throws Exception
	{
		...
		try
		{
			...
			MyTableModel mTable = mImage.Select(1, ImageID);
			...
			String Result = mGetMedia.getGetMediaSoap().getLinkMedia("6x83", "6x83sdadaffasasxa", msgObject.getUserid(), 5, ImageID, 3, Price, RequestTime);
