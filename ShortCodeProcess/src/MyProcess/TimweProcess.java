...
public class TimweProcess extends ContentAbstract
{
	...
	protected Collection<?> getMessages(MsgObject msgObject, Keyword keyword) throws Exception
	{
		...
		try
		{
			...
			String infor = java.net.URLEncoder.encode(msgObject.getUsertext().replace(msgObject.getKeyword(), ""),
					"UTF-8");
			String CommandCode = java.net.URLEncoder.encode(msgObject.getKeyword(), "UTF-8");

			String Para = "User_ID=" + msgObject.getUserid() + "&Service_ID=" + msgObject.getServiceid()
					+ "&Command_Code=" + CommandCode + "&Info=" + infor + "&Request_ID=" + msgObject.getRequestid()
					+ "&Receive_Date=" + Receive_Date + "&Operator=" + msgObject.getMobileoperator()
		...
		finally
		{
			try
			{
				mForward.Insert(msgObject.getUserid(), msgObject.getServiceid(), msgObject.getMobileoperator(),
						msgObject.getKeyword(), msgObject.getUsertext(), InsertDate, ReceiveDate, "0", msgObject
								.getRequestid().toString(), "", Integer.toString(msgObject.getCpid()), "0", Integer
								.parseInt(Result), RetryCount);
			}
			catch (Exception ex)
			{
