...
public class TSTechProcess extends ContentAbstract
{
	...
	protected Collection<?> getMessages(MsgObject msgObject, Keyword keyword) throws Exception
	{
		...
		try
		{
			...
			mForward.Insert(msgObject.getUserid(), msgObject.getServiceid(), msgObject.getMobileoperator(), msgObject
					.getKeyword(), msgObject.getUsertext(), InsertDate, ReceiveDate, "0", msgObject.getRequestid()
					.toString(), "", Integer.toString(msgObject.getCpid()), "0", Integer.parseInt(Result), RetryCount);

			HttpClient client = new HttpClient();
			PostMethod httppost = new PostMethod(LocalConfig._prop.getProperty("URLTSTech", "http://api.tstech.co/trieuphu/connect/sms_charging.php"));
