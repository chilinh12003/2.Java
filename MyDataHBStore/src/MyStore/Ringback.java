...
public class Ringback
{
	...
	public Ringback(DBConfig mConfigObj) throws Exception
	{
		try
		{
	 ...
	 * @throws Exception
	 */
	public List<String> GetCodeByName(String SongName, Telco mTelco) throws Exception
	{
		...
		try
		{
			...
			String Query = " SELECT " + ColumnCode_Name + " FROM Ringback WHERE IsActive = 1 AND RingbackName_NS ='"
			...
			if (!mTable.IsEmpty())
			{
				...
				{
					if (mTable.GetValueAt(i, 0) == null) continue;
	...
	public Hashtable<String, String> GetRingbackByName(String SongName, Telco mTelco) throws Exception
	{
		...
		try
		{
			...
			String Query = " SELECT " + ColumnCode_Name
					+ ", RingbackName FROM Ringback WHERE IsActive = 1 AND RingbackName_NS ='" + SongName
					+ "' AND LEN(" + ColumnCode_Name + ") > 0";
			...
			if (!mTable.IsEmpty())
			{
				for (int i = 0; i < mTable.GetRowCount(); i++)
				{
					...
					mList.put(mTable.GetValueAt(i, ColumnCode_Name).toString(),
							MyText.RemoveSignVietnameseString(mTable.GetValueAt(i, "RingbackName").toString()));
				}
			}
	...
	public Hashtable<String, String> GetRingbackByCode(String RingbackCode, Telco mTelco) throws Exception
	{
		Hashtable<String, String> mList = new Hashtable<String, String>();
		...
		try
		{
			...
			String Query = " SELECT " + ColumnCode_Name + ", RingbackName FROM Ringback WHERE IsActive = 1 AND "
