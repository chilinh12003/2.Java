...
public class EMSBuilderfromQueue extends Thread
{
	...
	public EMSBuilderfromQueue(Queue toSMSC, Queue EMSQueue, Map Wait4ResponseTable, Queue blacklistQueue)
	{
		this.toSMSC = toSMSC;
	...
	public void run()
	{
		...
		while (Gateway.running)
		{
			try
			{
				if (EMSQueue.size() > -1)
				{
					Logger.info("{EMSQueue=" + this.getName() + "}{1.EMSQueue.size=" + EMSQueue.size() + "}");
				}
				EMSData ems = (EMSData) EMSQueue.dequeue();

				if (EMSQueue.size() > -1)
				{
					Logger.info("{EMSQueue=" + this.getName() + "}{2.EMSQueue.size=" + EMSQueue.size() + "}");
				}
			...
			{
				Gateway.util.logErr(this.getClass().getName(), "{InterruptedException}" + ex.getMessage());
			}
			catch (DBException ex)
			{ // when lost connection to db
				Gateway.util.logErr(this.getClass().getName(), "{DBException}" + ex.getMessage());
			}
			catch (Exception ex)
			{
				Gateway.util.logErr(this.getClass().getName(), "EMSBuilderfromQueue:: " + ex.getMessage());
			}
		} // while
	...
	public int sendEMSQueueEx(EMSData ems) throws DBException, EMSException
	{
		...
		try
		{
			...
			{ // removed by other processes
				Gateway.util.log(this.getClass().getName(), "{EMSBuilderfromQueue: inValid ContentType}{ems is null}");
			}
			...
			{
				SubmitSM ssm = SMSCTools.buildSubmitSM(ems.getServiceIdEx(Constants.SERVICEID_FORMAT_SHORTCODE),
						ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL), ems.getText(), // text
																								// max
																								// 160
																								// chars
						ems.getId().intValue(), // seq = <emsId><segment_seqnum>
						registeredDelivery, ems.getMessageType(), ems.getCommandCode(), ems.getRequestId());

				synchronized (Wait4ResponseTable)
				{
					this.Wait4ResponseTable.put((ems.getId().intValue()) + "", ems);
				}
				Gateway.util.log(this.getClass().getName(), "{Add2Wait4ResponseTable}{ems=" + ems.getId() + "}"
			...
			else if ((ems.getContentType() == Constants.CT_WAP_SI))
			{
				if ("SFONE".equalsIgnoreCase(ems.getMobileOperator()))
				{
					...
					{
						Gateway.util.logErr(this.getClass().getName(), "{User_ID=" + ems.getUserId() + "}{Invalid SI:"
								+ ems.getText() + "}");
					}
					Collection vSubmits = EMSTools.buildSubmitEMS(otaData,
							ems.getServiceIdEx(Constants.SERVICEID_FORMAT_SHORTCODE),
							ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL), ems.getContentType(), ems.getId(),
							registeredDelivery, ems.getMessageType(), ems.getRequestId(), ems.getCommandCode());
					...
					for (Iterator it2 = vSubmits.iterator(); it2.hasNext(); i++)
					{
						...
						if (i == 1)
						{
							synchronized (Wait4ResponseTable)
							{
								this.Wait4ResponseTable.put((ssm.getSequenceNumber()) + "", ems);
							}
							Gateway.util.log(this.getClass().getName(),
									"{Add2Wait4ResponseTable}{ems=" + ssm.getSequenceNumber() + "}" + "{RequestID="
											+ ems.getRequestId() + "}");
				...
				else
				{
					...
					{
						String sMsg = sSi.substring(0, dot_idx);
						Gateway.util.log(this.getClass().getName(), "msg=" + sMsg);
						String sUrl = sSi.substring(dot_idx + 1);
						Gateway.util.log(this.getClass().getName(), "url=" + sUrl);
						SIgsm si = new SIgsm();
						si.setURL(sUrl);
						si.setMessage(sMsg);
						si.encodeSI();
					...
					else
					{
						...
						Gateway.util.log(this.getClass().getName(), "invalid SI: " + ems.getText());
						infoError = true;
					}
			...
			else if (ems.getContentType() == Constants.CT_BINARY)
			{
				...
				SubmitSM ssm = EMSTools.buildSubmitEMSfromBIN(ota2build,
						ems.getServiceIdEx(Constants.SERVICEID_FORMAT_SHORTCODE),
						ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL), ems.getId(), registeredDelivery,
						ems.getMessageType(), ems.getCommandCode(), 1, ems.getRequestId());
				synchronized (Wait4ResponseTable)
				{
					this.Wait4ResponseTable.put(((ems.getId().intValue()) + "") + "1", ems);
				}
				Gateway.util.log(this.getClass().getName(), "{Add2Wait4ResponseTable}{ems=" + ems.getId() + "1}"
						+ "{RequestID=" + ems.getRequestId() + "}");
				this.toSMSC.enqueue(ssm);
				time = new Timestamp(System.currentTimeMillis());
				ems.setSubmitDate(time);
			...
			{
				SubmitSM ssm = EMSTools.buildSubmitBanking(ems.getText(),
						ems.getServiceIdEx(Constants.SERVICEID_FORMAT_SHORTCODE),
						ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL), ems.getId(), registeredDelivery,
						ems.getMessageType(), ems.getCommandCode(), ems.getTotalSegments(), ems.getsRequestID());
				time = new Timestamp(System.currentTimeMillis());
				synchronized (Wait4ResponseTable)
				{
					this.Wait4ResponseTable.put((ems.getId().intValue()) + "" + "1", ems);
			...
			{
				SubmitSM ssm = EMSTools.buildSubmit411A(ems.getText(),
						ems.getServiceIdEx(Constants.SERVICEID_FORMAT_SHORTCODE),
						ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL), ems.getId(), registeredDelivery,
						ems.getMessageType(), ems.getCommandCode(), ems.getTotalSegments(), ems.getsRequestID());
				time = new Timestamp(System.currentTimeMillis());
			...
			{
				SubmitSM ssm = EMSTools.buildSubmitDPORT(ems.getText(),
						ems.getServiceIdEx(Constants.SERVICEID_FORMAT_SHORTCODE),
						ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL), ems.getId(), registeredDelivery,
						ems.getMessageType(), ems.getCommandCode(), ems.getTotalSegments(), ems.getsRequestID());
				time = new Timestamp(System.currentTimeMillis());
			...
			else if (ems.getContentType() == Constants.CT_TEXT_LONG)
			{
				...
				Collection vSubmits = EMSTools.buildSubmitLongMsg(ems.getText(),
						ems.getServiceIdEx(Constants.SERVICEID_FORMAT_SHORTCODE),
						ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL), ems.getContentType(), ems.getId(),
						registeredDelivery, ems.getMessageType(), ems.getCommandCode());
			...
			else if (ems.getContentType() == Constants.CT_OMA_LONG)
			{
				...
				Collection vSubmits = EMSTools.buildSubmitOMA(ems.getText(),
						ems.getServiceIdEx(Constants.SERVICEID_FORMAT_SHORTCODE),
						ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL), ems.getContentType(), ems.getId(),
						registeredDelivery, ems.getMessageType(), ems.getCommandCode());
			...
			else
			{
				...
				{
					Gateway.util.log(this.getClass().getName(), "Both Info and Raw_Info are null");
					infoError = true;
				}
				...
				{
					Gateway.util.log(this.getClass().getName(),
							"infoError: " + ems.getUserId() + ":" + ems.getCommandCode() + ":" + ems.getRequestId()
									+ ":" + ems.getText());
				...
				else
				{
					...
					Collection vSubmits = EMSTools.buildSubmitEMS(otaData,
							ems.getServiceIdEx(Constants.SERVICEID_FORMAT_SHORTCODE),
							ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL), ems.getContentType(), ems.getId(),
							registeredDelivery, ems.getMessageType(), ems.getCommandCode(), ems.getTotalSegments());
		...
		{
			Gateway.util.logErr(this.getClass().getName(), "Err:" + ex.getMessage() + "{ems=" + ems.getId() + "}"
					+ "{RequestID=" + ems.getRequestId() + "}{" + ems.getUserId() + "}");
		}
