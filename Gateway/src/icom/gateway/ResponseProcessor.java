...
public class ResponseProcessor extends Thread
{
	...
	public void run()
	{
		...
		while (Gateway.running)
		{
			...
			{
				Gateway.util.logErr(this.getClass().getName(), "ResponseProcessor.run: " + e.getMessage());
			}
		}
		// /////////////////////////////
		Gateway.util.log(this.getClass().getName(), "{ResponseProcessor stoped}");
		this.destroy();
		// /////////////////////////////
	}
	...
	private void processResponse(PDU pdu) throws Exception
	{
		...
		if (pdu.getCommandId() == Data.SUBMIT_SM_RESP)
		{
			...
			{
				Gateway.util.log(this.getClass().getName(), "{Respond for MT}{MessageId=" + ssmr.getSequenceNumber()
						+ "}");
				synchronized (wait4ResponseTable)
				{
					EMSData ems = (EMSData) wait4ResponseTable.remove(ssmr.getSequenceNumber() + "");

					if (ems == null)
					{
						Gateway.util.logErr(this.getClass().getName(),
								"{wait4ResponseTable: ems==null}{emsId=" + ssmr.getSequenceNumber() + "}{MessageID="
										+ ssmr.getMessageId() + "}");
						return;
					}
			...
			else
			{
				...
				if (this.seqNumber < 10)
				{ // =1: response for Invalid message
					Gateway.util
							.log(this.getClass().getName(), "ResponseProcessor: seqNumber < 10, =" + this.seqNumber);
				...
				synchronized (wait4ResponseTable)
				{
					...
					if (ems == null)
					{
						Gateway.util.log(this.getClass().getName(), "{processResponse: ems==null}{emsId="
								+ this.seqNumber + "}{MessageID=" + ssmr.getSequenceNumber() + "}");
						return;
					}
					ems.setNotes(ems.getNotes() + ":" + sErr);
					Gateway.util.log(this.getClass().getName(), "{Response for MT}{Request_ID=" + ems.getRequestId()
							+ "}{User_ID=" + ems.getUserId() + "}{Message_ID=" + ems.getMessageId() + "}");
					// Last segment ?
					...
					if ((resend == 1) && (ems.getSendNum() < Preference.NumOfRetries))
					{
						...
						Gateway.util.log(
								...
								"{Add MT to Resend Queue}{Request_ID=" + ems.getRequestId() + "}{User_ID="
										+ ems.getUserId() + "}{Message_ID=" + ems.getMessageId() + "}{Resend num="
										+ ems.getSendNum() + "}");
						ResendQueue.enqueue(ems);
					}
					else
					{
						Gateway.util.log(this.getClass().getName(), "{MT not Resend}{Request_ID=" + ems.getRequestId()
								+ "}{User_ID=" + ems.getUserId() + "}{Message_ID=" + ems.getMessageId()
								+ "}{Resend num=" + ems.getSendNum() + "}");
		...
		{
			Gateway.util.logErr(this.getClass().getName(), "processResponse (not processed).");
		}
		// Process response
		if (pdu.isGNack())
		{
			Gateway.util.logErr(this.getClass().getName(), "GENERIC_NAK (not processed).");
		}
	}
}
