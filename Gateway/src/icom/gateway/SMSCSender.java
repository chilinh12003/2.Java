		...
		while (Gateway.running)
		{
			if (Gateway.bound)
			{
				try
				{
					...
					if (pdu != null)
					{
						...
						else if (pdu.isRequest())
						{
							...
							int time2sleep = (1000 / Preference.maxSMPerSecond);

							// time2sleep = (1000 / Preference.maxSMPerSecond);
							this.sleep(time2sleep); // 10msg/sec --> 0.1sec
													// or
							// 100
							// }
						}
					...
					else
					{
						...
						DBTools.ALERT("SMSCSender", "pdu is null", Constants.ALERT_WARN, Preference.Channel
								+ "pdu is null", Preference.ALERT_CONTACT);
						this.sleep(50);
				...
				catch (InterruptedException ex1)
				{
					...
					DBTools.ALERT("SMSCSender", "InterruptedException", Constants.ALERT_WARN, Preference.Channel
							+ "InterruptedException: " + ex1.getMessage(), Preference.ALERT_CONTACT);
				...
				catch (ValueNotSetException ex)
				{
					...
					DBTools.ALERT("SMSCSender", "ValueNotSetException", Constants.ALERT_MAJOR, Preference.Channel
							+ "ValueNotSetException: " + ex.getMessage(), Preference.ALERT_CONTACT);
				...
				catch (IOException ex)
				{
					...
					DBTools.ALERT("SMSCSender", "IOException", Constants.ALERT_SERIOUS, Preference.Channel
							+ "IOException: " + ex.getMessage(), Preference.ALERT_CONTACT);
				...
				catch (Exception ex)
				{
					...
					DBTools.ALERT("SMSCSender", "Exception", Constants.ALERT_SERIOUS, Preference.Channel
							+ "Exception: " + ex.getMessage(), Preference.ALERT_CONTACT);
	...
	private void sendRequest(PDU pdu) throws IOException
	{
		...
		if (pdu.getCommandId() == Data.SUBMIT_SM)
		{
			try
			{
				...
				Gateway.util.log(this.getClass().getName(), "{toSMSC size= " + toSMSC.size() + "}{User_ID= "
						+ request.getDestAddr().getAddress() + "}{Service_ID=" + request.getSourceAddr().getAddress()
						+ "}{Info = " + info + "}");
			...
			catch (WrongSessionStateException ex)
			{
				...
				DBTools.ALERT("SMSCSender", "WrongSessionStateException", Constants.ALERT_SERIOUS, Preference.Channel
						+ "WrongSessionStateException: " + ex.getMessage(), Preference.ALERT_CONTACT);
			}
			catch (PDUException ex)
			{
				Gateway.util.log(this.getClass().getName(), "PDUException: " + ex.getMessage());

				DBTools.ALERT("SMSCSender", "PDUException", Constants.ALERT_SERIOUS, Preference.Channel
						+ "PDUException: " + ex.getMessage(), Preference.ALERT_CONTACT);
			...
			catch (TimeoutException ex)
			{
				...
				DBTools.ALERT("SMSCSender", "TimeoutException", Constants.ALERT_SERIOUS,
						Preference.Channel + "TimeoutException: " + ex.getMessage() + "\n"
			...
			catch (Exception ex)
			{
				...
				DBTools.ALERT("SMSCSender", "Exception", Constants.ALERT_SERIOUS, Preference.Channel + "Exception: "
						+ ex.getMessage(), Preference.ALERT_CONTACT);
